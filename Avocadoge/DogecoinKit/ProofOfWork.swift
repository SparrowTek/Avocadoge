...
//  Avocadoge
//
//  Created by Thomas Rademaker on 2/19/21.
//
...
struct ProofOfWork {
    ...
    var fPowAllowMinDifficultyBlocks = false
    var nPowTargetSpacing = 0
    
    private func allowMinDifficultyForBlock(pindexLast: CBlockIndex, pblock: CBlockHeader) -> Bool {
        // check if the chain allows minimum difficulty blocks
        guard fPowAllowMinDifficultyBlocks else { return false }
        
        // Dogecoin: Magic number at which reset protocol switches
        // check if we allow minimum difficulty at this block-height
        guard pindexLast.nHeight >= 157500 else { return false }
        
        // Allow for a minimum block time if the elapsed time > 2*nTargetSpacing
        return pblock.blockTime > pindexLast.blockTime + nPowTargetSpacing * 2
    }
